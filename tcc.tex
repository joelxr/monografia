...
\section{Descrição técnica}
 ...
 A ferramenta é escritas na linguagem \texttt{Java} e utiliza o \emph{framework} \texttt{JavaFX} para a interface do usuário, todo o código escrito para a primeira versão da ferramenta foi reutilizado, incrementando-o a medida que foi preciso na implementação do módulo de \textit{preview}.
...
Lembrar que, passar para o usuário o que uma determinada configuração representa também é papel dessa área.

Os painéis são mostrados na ferramenta de forma semelhante aos objetos na pré visualização, ou seja, ocorre através da árvore de objetos que, a medida que o usuário seleciona um objeto nela, exibe a configuração correspondente na região destinada aos painéis.
...
Usa-se o componente de acordeão para agregar as configurações no painel de acordo com o seu temas, ver a figura \ref{fig:painel_ator} por exemplo, nela temos um painel de configuração de um ator do jogo, pode-se perceber cada configuração separada, notar que há uma aba no acordeão para configurar as animações e outra para configurar as propriedades relacionadas a colisão.
