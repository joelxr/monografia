...
\begin{table}[h]
   ...
   \begin{tabular}{| l | p{8cm} |}
      \hline
      \textbf{Menu} & Assim como na primeira versão, armazena comandos da ferramenta, a partir do menu pode-se adicionar objetos no jogo, na nova versão da ferramenta o menu substitui a \emph{toolbar} e possui atalhos para facilitar o uso.  \\
      \hline
      \textbf{Área de pré visualização} & Região  destinada a prévia do jogo, onde os objetos são exibidos da forma mais próxima possível ao jogo.\\
      \hline
      \textbf{Árvore de objetos} & Componente que sumariza todos os objetos existentes no jogo. \\
      \hline
      \textbf{Painel de configuração} & Painel de configuração do objeto que foi selecionado na árvore. \\
      \hline
   \end{tabular}
...
Vale ressaltar que, a cada interação na árvore, a ferramenta atualiza os painéis e a área de pré visualização dependendo do objeto que esteja selecionado. Além disso, a ferramenta atualiza a prévia quando algumas propriedades são alteradas no painel. Por exemplo, o campo correspondente ao plano de fundo do mundo mostrado na imagem \ref{fig:tela_inicial_2}.
...
A seguir veremos como cada um dos problemas enumerados anteriormente foram resolvidos com base na solução sugerida.

\section{Organização dos objetos}

Árvores são ideais para organizar elementos que estão classificados hierarquicamente e tornou-se a melhor maneira de organizar os objetos da nova versão da ferramenta.
A versão anterior exibia os mundos criados como abas e dentro de cada aba, os objetos criados eram botões que - quando clicados - exibiam o painel de configuração correspondente.
Ao substituir essa visão pela atual, permite-se que, facilmente, o usuário tenha visão de tudo que existe no jogo sem a necessidade de alterar a visão da ferramenta.
Então, essa melhoria é mais lógica e trouxe mais praticidade a ferramenta.
A imagem \ref{fig:arvore_objetos} mostra um exemplo de uso da árvore de objetos para um jogo em criação.
